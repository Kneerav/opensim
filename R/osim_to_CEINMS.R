Osim_to_CEINMS = function(osim_model,
                          input_file,
                          output_file,
                          limb = "Right"){
  ...
  for(i in 1:length(muscles)){
    ...
    max = as.numeric(xmlValue(nodes_i[[1]][["max_isometric_force"]]))
    
    #Add
    Marker_R1 = newXMLNode("mtu", parent = nodes_y)
    newXMLNode(muscles[i],  name = "name", parent = Marker_R1)
    newXMLNode("-0.5",  name = "c1", parent = Marker_R1)
    newXMLNode("-0.5",  name = "c2", parent = Marker_R1)
    newXMLNode("0.5",  name = "shapeFactor", parent = Marker_R1)
